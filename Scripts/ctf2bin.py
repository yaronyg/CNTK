...
class Converter(object):
    ...
    def write_header(self, output):
        ...
        output.write(struct.pack('B', self.get_type()))
    ...
    def write_ints(self, output, ints):
...
class SparseConverter(Converter):
    ...
    def write_data(self, output):
        ...
        for sequence in self.sequences:
            ...
            self.write_ints(output, indices)
            self.write_ints(output, sizes)
...
def write_chunk(binfile, converters, chunk):
    ...
    # write out the (mas) sample length for each sequence in the chunk
...
class Header:
    ...
    def write(self, output_file):
        ...
        # First, write the magic number (unsigned long, 8 bytes)
        output_file.write(struct.pack('Q', MAGIC_NUMBER));
         # Next is the number of chunks (unsigned int, 4 bytes)
        output_file.write(struct.pack('I', len(self.chunks)))
        # Finally the number of inputs (unsigned int, 4 bytes)
        ...
        for chunk in self.chunks:
            ...
            # Uint32 Num sequences in the chunk
            output_file.write(struct.pack('I', chunk.num_sequences()))
            # Uint32 Num samples in the chunk
