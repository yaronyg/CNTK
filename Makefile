...
ifeq ($(ARCH),Darwin)
	ORIGINLIBDIR:='@executable_path/../lib'
	ORIGINDIR:='@executable_path'
else
...
PERF_PROFILER_LIB:= $(LIBDIR)/lib$(PERF_PROFILER).$(LIBEXT)
...
$(PERF_PROFILER_LIB): $(PP_OBJ)
	...
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,$(RPATH)%, $(ORIGINDIR)) -o $@ $^
...
$(CNTKMATH_LIB): $(MATH_OBJ) | $(PERF_PROFILER_LIB)
	...
	@echo creating $@ for $(ARCH) with build type $(BUILDTYPE)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,-L%, $(LIBPATH) $(LIBDIR) $(GDK_NVML_LIB_PATH)) $(patsubst %,$(RPATH)%, $(ORIGINDIR) $(LIBPATH)) -o $@ $^ $(LIBS) -fopenmp -l$(PERF_PROFILER)
...
CNTKLIBRARY_LIB:=$(LIBDIR)/lib$(CNTKLIBRARY).$(LIBEXT)
...
EVAL_LIB:=$(LIBDIR)/lib$(EVAL).$(LIBEXT)
ALL_LIBS+=$(EVAL_LIB)
SRC+=$(EVAL_SRC)
...
$(EVAL_LIB): $(EVAL_OBJ) | $(CNTKMATH_LIB)
	...
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,-L%, $(LIBDIR) $(LIBPATH) $(GDK_NVML_LIB_PATH)) $(patsubst %,$(RPATH)%, $(ORIGINDIR) $(LIBPATH)) -o $@ $^ $(LIBS) -l$(CNTKMATH) $(lMULTIVERSO) $(PROTOBUF_PATH)/lib/libprotobuf.a
...
HTKMLFREADER:=$(LIBDIR)/Cntk.Reader.HTKMLF-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
ALL_LIBS+=$(HTKMLFREADER)
SRC+=$(HTKMLFREADER_SRC)

$(HTKMLFREADER): $(HTKMLFREADER_OBJ) | $(CNTKMATH_LIB)
	@echo $(SEPARATOR)
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,-L%, $(LIBDIR) $(LIBPATH)) $(patsubst %,$(RPATH)%, $(ORIGINDIR) $(LIBPATH)) -o $@ $^ -l$(CNTKMATH)
...
COMPOSITEDATAREADER:=$(LIBDIR)/Cntk.Composite-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
HTKDESERIALIZERS:=$(LIBDIR)/Cntk.Deserializers.HTK-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
LMSEQUENCEREADER:= $(LIBDIR)/Cntk.Reader.LMSequence-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
LUSEQUENCEREADER:=$(LIBDIR)/Cntk.Reader.LUSequence-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
UCIFASTREADER:=$(LIBDIR)/Cntk.Reader.UCIFast-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
LIBSVMBINARYREADER:=$(LIBDIR)/Cntk.Reader.SVMBinary-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
SPARSEPCREADER:=$(LIBDIR)/Cntk.Reader.SparsePC-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
CNTKBINARYREADER:=$(LIBDIR)/Cntk.Deserializers.Binary-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
CNTKTEXTFORMATREADER:=$(LIBDIR)/Cntk.Deserializers.TextFormat-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
KALDI2READER:=$(LIBDIR)/Cntk.Reader.Kaldi2-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
ALL_LIBS+=$(KALDI2READER)
SRC+=$(KALDI2READER_SRC)

$(KALDI2READER): $(KALDI2READER_OBJ) | $(CNTKMATH_LIB)
	@echo $(SEPARATOR)
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,-L%, $(LIBDIR) $(KALDI_LIBPATH) $(LIBPATH)) $(patsubst %,$(RPATH)%, $(ORIGINDIR) $(KALDI_LIBPATH) $(LIBPATH)) -o $@ $^ -l$(CNTKMATH) $(KALDI_LIBS)
...
IMAGEREADER:=$(LIBDIR)/Cntk.Deserializers.Image-$(CNTK_COMPONENT_VERSION).$(LIBEXT)
...
$(IMAGEREADER): $(IMAGEREADER_OBJ) | $(CNTKMATH_LIB)
	@echo $(SEPARATOR)
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,-L%, $(LIBDIR) $(LIBPATH)) $(patsubst %,$(RPATH)%, $(ORIGINDIR) $(LIBPATH)) -o $@ $^ -l$(CNTKMATH) $(IMAGEREADER_LIBS)
endif
endif
...
MULTIVERSO_LIB:=$(LIBDIR)/libmultiverso.$(LIBEXT)
...
PYTHON_EXTRA_LIBS_BASENAMES:=$(addsuffix .$(LIBEXT),$(addprefix lib,$(filter-out $(PYTHON_LIBS_EXCLUDE_LIST),$(PYTHON_LIBS_LIST))))
