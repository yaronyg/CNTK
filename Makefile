...
COMPOSITEDATAREADER:=$(LIBDIR)/CompositeDataReader.$(LIBEXT)
ALL_LIBS+=$(COMPOSITEDATAREADER)
PYTHON_LIBS+=$(COMPOSITEDATAREADER)
SRC+=$(COMPOSITEDATAREADER_SRC)

$(LIBDIR)/CompositeDataReader.$(LIBEXT): $(COMPOSITEDATAREADER_OBJ) | $(CNTKMATH_LIB)
	@echo $(SEPARATOR)
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,-L%, $(LIBDIR) $(LIBPATH)) $(patsubst %,$(RPATH)%, $(ORIGINDIR) $(LIBPATH)) -o $@ $^ -l$(CNTKMATH)
...
HTKDESERIALIZERS:=$(LIBDIR)/HTKDeserializers.$(LIBEXT)
ALL_LIBS+=$(HTKDESERIALIZERS)
PYTHON_LIBS+=$(HTKDESERIALIZERS)
SRC+=$(HTKDESERIALIZERS_SRC)

$(LIBDIR)/HTKDeserializers.$(LIBEXT): $(HTKDESERIALIZERS_OBJ) | $(CNTKMATH_LIB)
	@echo $(SEPARATOR)
	$(CXX) $(LDFLAGS) ${SHAREDSWITCH} $(patsubst %,-L%, $(LIBDIR) $(LIBPATH)) $(patsubst %,$(RPATH)%, $(ORIGINDIR) $(LIBPATH)) -o $@ $^ -l$(CNTKMATH)
...
CNTKBINARYREADER:=$(LIBDIR)/CNTKBinaryReader.$(LIBEXT)
...
CNTKTEXTFORMATREADER:=$(LIBDIR)/CNTKTextFormatReader.$(LIBEXT)
