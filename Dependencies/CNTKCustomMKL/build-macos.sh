...
# TODO configurable
MKLROOT=/opt/intel/compilers_and_libraries_2017.2.163/mac/mkl
...
do
    ...
    make -f $MKLBUILDERROOT/makefile libuni \
        export=functions.txt \
        threading=$THREADING \
        name=$LIBBASENAME \
        MKLROOT=$MKLROOT
    mkdir Publish/$CNTKCUSTOMMKLVERSION/x64/$THREADING
    mv $LIBBASENAME.dylib Publish/$CNTKCUSTOMMKLVERSION/x64/$THREADING
done

cp -p $MKLROOT/../compiler/lib/libiomp5.dylib Publish/$CNTKCUSTOMMKLVERSION/x64/parallel
