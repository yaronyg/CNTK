...
namespace CNTK
{
    ...
    {
    public:
        CNTK_API CrossValidationConfig()
            : m_frequency(0),
            m_mbSize(MinibatchSizeSchedule(1))
        {}
    ...
    {
    public:
        CNTK_API CheckpointConfig() : m_frequency(0),
            m_restore(true),
            m_preserveAll(false)
        {}
    ...
    {
    public:
        CNTK_API ProgressConfig() : m_frequency(0)
        {}

        ///
        /// Sets progress printing configuration.
        /// progressWriters: list of progress writers that will be used for training.
        /// progressFrequency : an approximate number of global samples processed after which the OnProgress
        ///    callback is called
        ///
        CNTK_API ProgressConfig(const std::vector<ProgressWriterPtr>& progressWriters, size_t progressFrequency);

    private:
        friend class TrainingSession;
        const std::vector<ProgressWriterPtr> m_writers;
        const size_t m_frequency;
    };
