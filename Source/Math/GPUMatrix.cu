...
template <class ElemType>
void GPUMatrix<ElemType>::PSROIPoolingForward(const size_t numRois, const size_t numImg, const size_t groupSize, const size_t inChannels, const size_t outChannels, 
...
template <class ElemType>
void GPUMatrix<ElemType>::PSROIPoolingBackward(const size_t numRois, const size_t numImg, const size_t channels, const size_t width, const size_t height,
    const size_t pooledWidth, const size_t pooledHeight, const GPUMatrix<ElemType>& roiData, GPUMatrix<ElemType>& grad,
    GPUMatrix<ElemType>& workspace) const
{
    PrepareDevice();
    SyncGuard syncGuard;
    ...
    const int blockSize = GridDim::maxThreadsPerBlock;
    auto numThreads = dim3((int)floor((double)(count + blockSize - 1) / blockSize));
    auto groupChannels = channels / (pooledWidth * pooledHeight);
    kPSRoiPoolingBackward <<<numThreads, blockSize, 0, t_stream>>>(count, numRois, numImg, channels, groupChannels, width, height,
